...
export const useAppStore = create<AppState>((set, get) => ({
	...
	acceptChatRequest: async (userId) => {
		...
		if (!contact) {
			console.warn(
				`[Contacts] Accepting an unknown contanct is not possible: ${userId}`,
			);
			return;
		}
		...
		try {
			...
			console.info(
				`[Contacts] Accepted request from ${contact.username} (${contact.id})`,
			);
		} catch (err) {
	...
	declineChatRequest: async (userId, block = false) => {
		...
		if (!contact) {
			console.warn(
				`[Contacts] Rejecting an unknown contanct is not possible: ${userId}`,
			);
			return;
		}
